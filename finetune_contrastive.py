    ...
    def train(self):
        ...
        if self.config["feat_type"] == 'plus':
            ...
            if self.config['model_type'] == 'gin':
                from models.ginet_finetune_plus import GINet
            ...
            if self.config['model_type'] == 'gin':
                ...
                model = GCN(self.config['dataset']['task'], **self.config["model"]).to(self.device)
                model = self._load_pre_trained_weights(model)
        elif self.config["feat_type"] == 'concat':  # llm4sd ==> 512 (GNN dim)
...
if __name__ == "__main__":
    ...
    for iteration in [1]:  # Loop 10 times
        ...
        # Reset the results list for each iteration
        iteration_results = []
        ...
        for target in target_list:
            ...
            if args.knowledge_type == "synthesize":
                iteration_results.append([iteration, target, result, args.knowledge_type])
            else:
                iteration_results.append([iteration, target, result, args.knowledge_type, args.num_samples])
                if 'num_samples' not in headers:  # Only append once
        ...
        if not os.path.isfile(file_path):
            ...
            df = pd.DataFrame(iteration_results, columns=headers)
        ...
        # Clear the results list for the next iteration
        iteration_results = []